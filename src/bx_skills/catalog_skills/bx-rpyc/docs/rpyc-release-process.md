...
2.  Describe commit history within `CHANGELOG.rst` (see [Generate Entry](#generate-changelogrst-entry))
3.  Update `release_date` in `rpyc/version.py` and bump version ([Semantic Versioning](https://semver.org/) and [Versioning using Hatch](https://hatch.pypa.io/latest/version/))
...
11. Create new release such that the notes are from <span class="title-ref">CHANGELOG.rst</span> entry (`` %s/`#/#/g `` and `` %s/`_//g ``)
...
Once insert this entry at the top of <span class="title-ref">CHANGELOG.rst</span>, review what it looks like with <span class="title-ref">instant-rst</span>.
...
- [Wheel file name convention](https://peps.python.org/pep-0427/#file-name-convention)
