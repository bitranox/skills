...
- `SLAVE` indicates the `SlaveService` (you'll learn more about [services](../docs/services.md) later on), and
...
Note that currently it is not possible to teleport arbitrary functions, in particular there can be issues with closures to non-trivial objects. In case of problems it may be worth taking a look at external libraries such as [dill](https://pypi.org/project/dill/).

Continue to [Part 2](tut2.md)...
