...
In [Part 1](tut1.md), we have seen how to use rpyc classic connection to do almost anything remotely.
...
>
```
>>> type(conn.modules.os.path.abspath) <netref class 'builtins.function'>
```
...
In addition to some expected methods and properties, you will have noticed `____conn__` and `____id_pack__`. These properties store over which connection the object should be resolved and an identifier that allows the server to lookup the object from a dictionary.
