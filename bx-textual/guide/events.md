...
By way of an example, let's consider what happens if you were to type "Text" into a `Input` widget. When you hit the `T` key, Textual creates a `key` event and sends it to the widget's message queue. Ditto for `E`, `X`, and `T`.
...
![Event queue](../images/events/queue.excalidraw.svg)
...
![Event queue idle](../images/events/queue2.excalidraw.svg)
...
![Event naming](../images/events/naming.excalidraw.svg)
...
- [Events reference](../events/index.md) - Complete list of built-in events
- [Input](./input.md) - Keyboard, mouse, and focus events
- [Actions](./actions.md) - Respond to user interactions with actions
- [Workers](./workers.md) - Run long-running tasks without blocking the UI
- [Widgets](./widgets.md) - Build custom widgets that emit messages
