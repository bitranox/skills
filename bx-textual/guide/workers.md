...
If you need the return value you can call `worker.wait` which is a coroutine that will wait for the work to complete.
...
You can cancel a worker at any time before it is finished by calling `Worker.cancel`.
This will raise a `CancelledError` within the coroutine, and should cause it to exit prematurely.
...
Worker objects have a `state` attribute which will contain a `WorkerState` enumeration that indicates what the worker is doing at any given time.
...
![Worker lifetime](../images/workers/lifetime.excalidraw.svg)
...
You can work around this with the `App.call_from_thread` method which runs your function from the main thread.
...
Note the use of `get_current_worker` which the function uses to check if it has been cancelled or not.
...
An exception would be `post_message` which *is* thread-safe.
...
- [Events and Messages](./events.md) - Custom messages for worker-to-UI communication
- [Screens](./screens.md) - Manage screens with background tasks
- [Dev Tools](./devtools.md) - Debug workers with the Textual console
