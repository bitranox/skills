...
The *content* of widget (displayed within the widget's borders) is typically specified in a call to ``Static.update`` or returned from ``render()`` in the case of [custom widgets](./widgets.md#custom-widgets).
...
Colors may specified in the same way as a CSS [<color>](../css_types/color.md).
...
You can also any of the [named colors](../css_types/color.md).
...
The `Content` class accepts a default string in it's constructor.
...
If you want markup, you can create a `Content` with the `Content.from_markup` alternative constructor:
...
You can add styles to content with the `stylize` or `stylize_before` methods.
...
- [Widgets](./widgets.md) - Build custom widgets with render methods
- [Design](./design.md) - Theme colors and CSS variables for use in markup
- [CSS color type](../css_types/color.md) - Color values for markup and styles
- [Static widget](../widgets/static.md) - Display static text content
