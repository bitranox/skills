...
After installing, you can set the ``language`` reactive attribute on the `TextArea` to enable highlighting.
...
The content of the `TextArea` can be updated using the ``replace`` method.
...
The cursor location is available via the ``cursor_location`` property, which represents
...
For example, ``cursor_at_start_of_line`` tells us if the cursor is at a start of line.

We can also check the location the cursor _would_ arrive at if we were to move it.
For example, ``get_cursor_right_location`` returns the location
...
The ``move_cursor`` method allows you to move the cursor to a new location while selecting
...
The ``move_cursor_relative`` method offers a very similar interface, but moves the cursor relative
...
After creating a `TextArea`, you can change the theme by setting the ``theme``
...
You can manually add a checkpoint by calling the ``TextArea.history.checkpoint()`` instance method.
...
*API reference: `textual.widgets._text_area.TextArea.BINDINGS`*
...
*API reference: `textual.widgets.TextArea.COMPONENT_CLASSES`*


## See also

- [Input](./input.md) - Single-line text input widget
...
*API reference: `textual.widgets._text_area.TextArea`*


---

*API reference: `textual.widgets.text_area`*
